...
Ein wichtiger Punkt ist für mich, dass ich auch meine Filme auf Datenträgern katalogisieren kann.  Dafür habe ich ein eigenes Verzeichnis erstellt, was eine Frage des Modus Operandi ist. Für eine Medien Verwaltungssoftware spielt es keine Rolle. Anstatt das eine AVI oder MKV in einem Verzeichnis eingelesen wird, wird eine Textdatei mit der Erweiterung ".DISC" erstellt.
...
* ![Bild1](../images/tinymediamanager/tmm-cnom-5.jpg) 
* ![Bild1](../images/tinymediamanager/tmm-cnom-6.jpg) 
...
Über die Powershell CLI starte ich dieses Skript ("mkfilm.ps1") aus dem Skriptverzeichnis mit der Angabe des Namens des Films. Beispiel: ``.\mkfilm.ps1 "Reine Nervensache (1999)"`` Die Anführungsstriche ( " ") sind notwendig, weil die Kommandozeile (CLI) den Leerschlag als Abtrennung interpretiert. D.h. nicht als ganzer Text. Jedem bekannt, der  mit der Kommandozeile (CMD / Powershell) arbeitet.  
Unbedingt beachten, welche Sonderzeichen vom Dateisystem unterstützt werden. Z.B. kann ein Doppelpunkt für ein NTFS Dateisystem (Windows) nicht verwendet werden. Das Skript kopiert die die Datei Vorlage.disc in das neu erstellte Filmverzeichnis und benennt es analog zum Verzeichnisnamen. Da in diesem Beispiel nur BluRay Formate erfasst werden, habe ich den Präfix ".BR" in die Vorlage eingefügt. D.h. beim Beispiel oben, heisst die Textdatei im Verzeichnis "Reine Nervensache (1999).BR.disc". Natürlich kann man den Vermerk BR weglassen, wenn man diese Information nicht wünscht. Oder man gibt es dynamisch mit dem Filmtitel an, wofür der "Param" Block um eine Eingabe erweitert werden muss. Meine Arbeitsmethodik ist Stapelverarbeitung. D.h. periodisch packe ich eine Pendenz - sprich einen Stapel DVD - und erfasse sie. Daher benötige ich den zweiten Parameter mit dem Aufruf nicht. Ich ändere einfach in dem Skript den Wert ".BR" auf ".UBR" bzw. ".DVD".

Das Skript:  

````
# mkfilm.ps1
  # Filmverzeichnis f�r TinyMedia Manager erstellen

  #Verzeichnis und Datei erstellen
if ($args) {
    foreach ($datei in $args){
	if (test-path ("Z:\Film DISC\" + $datei) -pathType container) {
		write-host "Das Verzeichnis " + $datei + "existiert bereits"
        }
	else{
	        New-Item -Path "Z:\Film DISC" -Name $datei -ItemType directory
        	Copy-Item "Z:\Film DISC\vorlage.disc" ("Z:\Film DISC\" + $datei)
	        rename-item ("Z:\Film DISC\" + $datei + "\vorlage.disc") ($datei + ".DVD" + ".disc")
        }
     }
}
````

Die praktische Anwendung in einer Powershell Kommandozeile - ausgehend von dem Verzeichnis wo sich die Skripte befinden, sieht so aus:  

![Bild1](../images/tinymediamanager/ps-1.jpg) 


# Quellen

* [Tinymedia Manager](http://www.tinymediamanager.org)
* [Community](https://forum.kodi.tv/forumdisplay.php?fid=204)
* [TMM Nightly BuildS](http://nightly.tinymediamanager.org)
